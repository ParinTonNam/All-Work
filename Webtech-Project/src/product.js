    ...
    return (shop.innerHTML = whereisproduct.map((x) => {
      ...
      function looptopping(num) {
        ...
        if (num == search.topping.length - 1) {
          return `<label class="switch"><input onclick="addchoice3(${search.id},this.id)" type="radio" name="topping" style="display:none" id="`+ search.topping[num] + `"><span class="slider">` + search.topping[num] + `</span></input></label>`;
        }
        else if (num < search.topping.length) {
          return `<label class="switch"><input onclick="addchoice3(${search.id},this.id)" type="radio" name="topping"  style="display:none" id="`+ search.topping[num] + `"><span class="slider">` + search.topping[num] + `</span></input></label>` + looptopping(num + 1);
        }
      }
      ...
      <label class="switch">
        <span class="slider">
          <a class="buy-btn" href="./index.html"><i class="bi bi-house-fill">Home</i></a>
        </span>
      </label>
      <label class="switch">
        <span class="slider">
          <a class="buy-btn" href="./cart.html"><i class="bi bi-cart2">Cart</i></a>
        </span>
      </label>
   </div>
    `;
...
let addchoice1 = (id,clicked_id) => {
  let selectedItem = id;
  let search = choicebasket.find((x) => x.id === selectedItem.id);
